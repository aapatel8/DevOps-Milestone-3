...
function callGetDropletInfo() {
	client.getDropletInfo(dropletId, function(error, response) {
		...
		exec('sudo apt-get update && sudo apt-get -y install python > /dev/null 2>&1', {
			...
			.pipe(process.stdout);
    fs.appendFileSync("roles/Checkbox_Stable/templates/variables", 'MONGO_IP=' + ip);
		fs.appendFileSync("roles/Checkbox_Canary/templates/variables", 'MONGO_IP=' + ip);
		var logger = fs.createWriteStream('inventory', {
			flags: 'w' // 'a' means appending (old data will be preserved)
		});
		logger.write('\n' + "[mongodb]\n" + ip + " ansible_ssh_user=root " + "ansible_ssh_private_key_file=/home/vagrant/.ssh/id_rsa\n");
