...
function callCreateDroplet() {
	var name = "redis-"+os.hostname() + index;
...
function callGetDropletInfo(instanceIndex) {
	client.getDropletInfo(dropletIds[instanceIndex], function(error, response) {
		...
		logger.write("root" + instanceIndex + " ansible_host=" + ip + " ansible_ssh_user=root" + " ansible_ssh_private_key_file=~/.ssh/id_rsa\n");
